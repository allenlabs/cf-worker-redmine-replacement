...
const CRON_SRC = path.resolve(ROOT, './workers/cron');
...
export default defineConfig({
  ...
  test: {
    ...
    projects: [
      {
        plugins: [react()],
        resolve: { alias: { '~': WEB_APP, '@api': API_SRC, '@cron': CRON_SRC } },
        test: {
          name: 'node',
          environment: 'node',
          include: [
    ...
    coverage: {
      ...
      exclude: [
        ...
        // smoke tests; the pure impls behind them live in
        // workers/web/app/server/*.ts which DO need coverage.
      ...
      thresholds: {
        // Match focus / inbox / context baseline.
        lines: 99,
        statements: 99,
        functions: 99,
        branches: 99,
