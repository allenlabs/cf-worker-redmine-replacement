...
/**
 * Session handling — verbatim port of context / focus / inbox session.server.ts
 * with the cookie name changed to `concierge_session`.
 *
 * Sign-in happens on auth.allen.company.  Concierge never sees the user's
 * password.  After /auth/callback exchanges the SSO code for an RS256 JWT,
 * we store the JWT in `concierge_session` and verify it via JWKS on every
