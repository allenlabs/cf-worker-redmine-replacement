...
//
// Concierge doesn't own the `users` table — `pm.users` is the source of truth
// for every allenlabs app.  We reach into it directly via raw SQL because
// Drizzle's schema graph is per-app.
 ...
 * null if no row exists yet or if the row is locked.  Used by every
 * authenticated request to map the cookie → user_id.
 */
