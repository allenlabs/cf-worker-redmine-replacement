...
describe('readSessionToken', () => {
  ...
  it('extracts the token', () => {
    expect(readSessionToken(`foo=bar; ${SESSION_COOKIE}=abc.def.ghi`)).toBe('abc.def.ghi');
  });
  it('handles `=` inside the token value', () => {
    expect(readSessionToken(`${SESSION_COOKIE}=aa=bb=cc`)).toBe('aa=bb=cc');
  });
...
describe('cookieHeader / clearCookieHeader', () => {
  it('uses the gentle_session cookie name', () => {
    expect(SESSION_COOKIE).toBe('gentle_session');
  });
  it('formats Set-Cookie correctly', () => {
    expect(cookieHeader('abc')).toMatch(/^gentle_session=abc; HttpOnly;/);
