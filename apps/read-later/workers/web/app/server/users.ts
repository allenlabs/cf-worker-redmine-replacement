...
//
// Read-later doesn't own the `users` table — `pm.users` is the source of
// truth for every allenlabs app (PM, inbox, focus, context, read-later, ...).
// We reach into it directly via raw SQL because Drizzle's schema graph is
// per-app.
 ...
 * cookie -> user_id for soft-FK writes into read_later.items.
 */
