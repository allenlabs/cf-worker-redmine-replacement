...
/**
 * Session handling — verbatim port of context / focus / inbox
 * session.server.ts with the cookie name changed to `read_later_session`.
 *
 * Sign-in happens on auth.allen.company.  Read-later never sees the user's
 * password.  After /auth/callback exchanges the SSO code for an RS256 JWT,
 * we store the JWT in `read_later_session` and verify it via JWKS on every
...
export const SESSION_COOKIE = 'read_later_session';
