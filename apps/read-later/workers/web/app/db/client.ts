...
//
// Mirrors the focus / inbox / context client verbatim (per-request WeakMap
// cache, retry-wrapped proxy on connection-shaped errors).  Pinned
// search_path = `read_later, public` so unqualified table refs in schema.ts
// resolve to our app schema.
//
...
function buildClient(env: { HYPERDRIVE: Hyperdrive }) {
  const raw = postgres(env.HYPERDRIVE.connectionString, {
    ...
    prepare: false,
    idle_timeout: 5,
    connection: { search_path: 'read_later, public' },
  });
