...
//
// Inbox doesn't own the `users` table — `pm.users` is the source of truth
// for every allenlabs app (PM, inbox, today, focus, ...).  We reach into
// it directly via raw SQL because Drizzle's schema graph is per-app: the
// inbox schema knows about `inbox.items` and `inbox.api_clients`, not
// `pm.users`.  This is fine — the soft FK convention is enforced by SSO,
// not by the DB.
 ...
 * cookie -> user_id for soft-FK writes into inbox.items.
 */
