// TanStack Start–aware helpers.  Mirror of PM's auth-runtime.server.ts but
// trimmed down — inbox is single-tenant per user with no project membership
// matrix, so we only need `getCurrentUser` / `requireUser`.
//
...
// Request-scoped dedupe (same pattern as PM): every per-request loader
// shares one Promise<user|null> via this WeakMap.  GC'd when the request
// exits.
