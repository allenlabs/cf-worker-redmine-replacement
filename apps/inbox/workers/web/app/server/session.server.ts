...
/**
 * Session handling — verbatim port of PM's session.server.ts.
 *
 * Sign-in happens on auth.allen.company.  Inbox never sees the user's
 * password.  After /auth/callback exchanges the SSO code for an RS256 JWT,
 * we store the JWT in `inbox_session` and verify it via JWKS on every
