...
describe('verifySessionToken', () => {
  ...
  it('returns null when AUTH_API_URL is missing', async () => {
    // Force a "missing AUTH_API_URL" path via an env override.
  ...
  it('builds a fresh JWKS for an issuer the cache has never seen', async () => {
    // primeJwks pre-seeds the cache for the default AUTH_API_URL.  By
    // verifying against a fresh URL we force getJwks() to fall through
    // its cache-miss branch and build a new createRemoteJWKSet() (which
    // will throw on the actual network fetch — that's the point: a fresh
    // URL must error rather than serving a stale cached JWKS).
