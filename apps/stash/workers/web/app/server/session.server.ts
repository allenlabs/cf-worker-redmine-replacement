...
/**
 * Session handling — verbatim port of focus / inbox / context session.server.ts
 * with the cookie name changed to `stash_session`.
 *
 * Sign-in happens on auth.allen.company.  Stash never sees the user's
 * password.  After /auth/callback exchanges the SSO code for an RS256 JWT,
 * we store the JWT in `stash_session` and verify it via JWKS on every
