...
// api/lib/db.ts — we deliberately do NOT reuse the web worker's `~/db/client`
// because that imports `@tanstack/react-start/server` to participate in the
// SSR per-request cache.  The API worker is plain Hono — no SSR runtime, no
