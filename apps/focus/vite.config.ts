...
// Same post-bypass shape as PM / inbox: we DON'T use @cloudflare/vite-plugin.
// Its worker env runs parallel to TanStack Start's `server` env and ships a
// stub `getServerFnById` resolver, so every POST /_serverFn/<hash> ends up
// 500.  TanStack Start's own `dist/server/server.js` is the real worker; the
...
export default defineConfig({
  plugins: [
    ...
    {
      name: 'focus-cf-worker-polyfills',
      apply: 'build',
      enforce: 'post',
      config() {
        return {
