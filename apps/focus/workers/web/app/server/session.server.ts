...
/**
 * Session handling — verbatim port of inbox's session.server.ts with the
 * cookie name changed to `focus_session`.
 *
 * Sign-in happens on auth.allen.company.  Focus never sees the user's
 * password.  After /auth/callback exchanges the SSO code for an RS256 JWT,
 * we store the JWT in `focus_session` and verify it via JWKS on every
