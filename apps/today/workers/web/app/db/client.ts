...
//
// Mirrors the inbox / focus client verbatim (per-request WeakMap cache,
// retry-wrapped proxy on connection-shaped errors).  Search path is left at
// the Hyperdrive default (`public`) and every query in today qualifies its
// tables explicitly — `pm.users`, `inbox.items`, `focus.sessions`, etc.
//
// See `apps/project-management/workers/web/app/db/client.ts` for the long
// rationale on why we can't cache a postgres() client at module level.
...
export function makeDb(env: { HYPERDRIVE: Hyperdrive }) {
  ...
  try {
    ...
    /* Outside a request context (tests, scheduled triggers, background
       ctx.waitUntil work) — fall through to per-call client. */
  }
  if (req) {
