...
// Public paths a signed-out browser must still hit cleanly.  Today is
// read-only and SSO-only, so the public list is small (auth bounce only).
...
export const Route = createRootRouteWithContext<RouterContext>()({
  ...
  loader: async () => {
    ...
    let req: Request | undefined;
    try { req = getRequest(); } catch { return { user: null, appName: 'Today', locale: DEFAULT_LOCALE }; }
    const cookie = req?.headers.get('cookie') ?? null;
    const token = readSessionToken(cookie);
    const env = getEnv();
