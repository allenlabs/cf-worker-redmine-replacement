...
/**
 * Session handling — verbatim port of inbox / focus' session.server.ts with
 * the cookie name changed to `today_session`.
 *
 * Sign-in happens on auth.allen.company.  Today never sees the user's
 * password.  After /auth/callback exchanges the SSO code for an RS256 JWT,
 * we store the JWT in `today_session` and verify it via JWKS on every
...
export const SESSION_COOKIE = 'today_session';
