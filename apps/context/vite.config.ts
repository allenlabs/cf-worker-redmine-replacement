...
// is the real worker; the only thing we still need from cf-vite-plugin is the
// `__name` esbuild helper polyfill, injected as a tiny banner so workerd
// boots clean.
export default defineConfig({
  plugins: [
    ...
    {
      name: 'context-cf-worker-polyfills',
      apply: 'build',
      enforce: 'post',
      config() {
        return {
